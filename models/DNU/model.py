...
import torchvision
from models.DNU.utils import *
import os
from models.DNU.utils_reid import *
...
class DNU_stage(nn.Module):
    def __init__(self, n_channels, L=64, k=1):
        ...
        self.S = SpectralPriorNet(n_channels, L).cuda()
        # adaptive parameter eta
        self.deta = torch.nn.Parameter(torch.FloatTensor([0.04])).cuda()
        self.eta = torch.nn.Parameter(torch.FloatTensor([0.8])).cuda()
    ...
    def forward(self, f, f0, g, Phi, PhiPhiT):
        ...
        t1 = A_torch(f, Phi).cuda() # [bs, row, col]
        t2 = At_torch(t1, Phi).cuda() # [bs, nC, row, col]
