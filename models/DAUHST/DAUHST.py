...
class DAUHST(nn.Module):
    ...
    def forward(self, y):
        ...
        input_mask = torch.Tensor([1.0]).unsqueeze(1).unsqueeze(1).unsqueeze(1)
        input_mask = input_mask.expand([1, 28, 256, 310]).cuda()
        Phi = input_mask

        Phi_s = torch.Tensor([1.0]).cuda()
        # Phi_s = torch.Tensor([1.0]).unsqueeze(1).unsqueeze(1).unsqueeze(1)
        # Phi_s = Phi_s.expand([1, 28, 256, 310]).cuda()      
